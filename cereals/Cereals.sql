...
create view see as (select name,mfr,calories,protein,fat,sodium,fiber,carbo,sugars,potass,vitamins,shelf,weight,cups,rating from cereals_table) ;
select * from see;
...
select type,count(*) No_of_cold_cereals from cereals_table where type = 'C';
...
select count(distinct(name)),count(*) from cereals_table;  

-- 9. Find average of calories of hot cereal and cold cereal in one query 
select type,round(avg(calories),2) as Average_calories from cereals_data group by type;
...
select *,case when calories >106.97 then 'High' else 'low' end HL_Calories from cereals_table;
alter table cereals_table add column HL_calories varchar(4);
select round(avg(calories),2) from cereals_table;
...
select * from cereals_table where HL_Calories = 'HIGH';
...
select type,round(avg(rating),2) from cereals_data group by type;
...
select mfr,count(*) as No_of_Manufacture from cereals_data group by mfr;
