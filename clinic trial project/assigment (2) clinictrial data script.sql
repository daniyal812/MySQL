...
select pregnant,round(avg(age),2) from clinictrial group by pregnant;

#7. Find average blood pressure of those had drug reaction and did not had drug
-- reaction
select drug_reaction,round(avg(bp),2) from clinictrial group by drug_reaction;
...
update clinictrial set age=32 where name = 'reerhika';
